...
import { arr, arrEmails, removeRecentEmail, getEmails, setCurrentEmail, pendingMobileNav, currentEmail, type Email } from 'src/store/current-email';
...
export default function Items({ email }: { email?: string }) {
  ...
  if ($arr.length === 0) {
    if ($arrEmails.length > 0) return <RecentEmails />;
    return (
